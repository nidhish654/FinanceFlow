"use client";

import { AlertTriangle, XCircle, Clock } from "lucide-react";

import {
    Card,
    CardContent,
    CardHeader,
    CardTitle,
} from "@/components/ui/card";

import { Progress } from "@/components/ui/progress";
import { AnalyticsBudget } from "@/features/analytics/types/analytics-view";
import { cn } from "@/lib/utils";

interface BudgetNeedsAttentionProps {
    budgets: AnalyticsBudget[];
    currency: string;
}

export default function BudgetNeedsAttention({
    budgets,
    currency,
}: BudgetNeedsAttentionProps) {
    const today = new Date();
    today.setHours(0, 0, 0, 0);

    // Filter budgets that need attention
    const atRisk = budgets
        .filter((b) => {
            if (b.isExceeded) return true;
            if (b.utilization >= 88) return true;
            
            // Check if active and ends within 3 days
            const end = new Date(b.endDate);
            end.setHours(0, 0, 0, 0);
            const start = new Date(b.startDate);
            start.setHours(0, 0, 0, 0);
            
            const isActive = start <= today && end >= today;
            if (isActive) {
                const daysLeft = Math.ceil((end.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
                if (daysLeft <= 3 && daysLeft >= 0) return true;
            }
            
            return false;
        })
        .sort((a, b) => b.utilization - a.utilization); // Sort worst first

    if (atRisk.length === 0) return null;

    const formatCurrency = (value: number) =>
        new Intl.NumberFormat("en-IN", {
            style: "currency",
            currency,
            maximumFractionDigits: 0,
        }).format(value);

    const getStatusInfo = (b: AnalyticsBudget) => {
        if (b.isExceeded) {
            return {
                label: "Exceeded",
                badge: "bg-red-500/10 text-red-600 dark:text-red-400",
                bar: "bg-red-500",
                pct: "text-red-600 dark:text-red-400",
                icon: <XCircle className="h-4 w-4 text-red-500" />,
            };
        }
        
        if (b.utilization >= 95) {
            return {
                label: "Almost Full",
                badge: "bg-amber-500/10 text-amber-600 dark:text-amber-400",
                bar: "bg-amber-500",
                pct: "text-amber-600 dark:text-amber-400",
                icon: <AlertTriangle className="h-4 w-4 text-amber-500" />,
            };
        }

        if (b.utilization >= 88) {
            return {
                label: "Warning",
                badge: "bg-amber-500/10 text-amber-600 dark:text-amber-400",
                bar: "bg-amber-500",
                pct: "text-amber-600 dark:text-amber-400",
                icon: <AlertTriangle className="h-4 w-4 text-amber-500" />,
            };
        }

        // Must be ending soon
        const end = new Date(b.endDate);
        end.setHours(0, 0, 0, 0);
        const daysLeft = Math.ceil((end.getTime() - today.getTime()) / (1000 * 60 * 60 * 24));
        const label = daysLeft === 0 ? "Ends Today" : `Ends in ${daysLeft} days`;

        return {
            label,
            badge: "bg-blue-500/10 text-blue-600 dark:text-blue-400",
            bar: "bg-blue-500",
            pct: "text-blue-600 dark:text-blue-400",
            icon: <Clock className="h-4 w-4 text-blue-500" />,
        };
    };

    return (
        <Card className="rounded-2xl border shadow-sm border-amber-500/20 bg-amber-500/5">
            <CardHeader className="pb-3">
                <CardTitle className="flex items-center gap-2 text-amber-600 dark:text-amber-400">
                    <AlertTriangle className="h-5 w-5" />
                    Needs Attention
                </CardTitle>
            </CardHeader>
            <CardContent>
                <div className="space-y-4">
                    {atRisk.map((budget) => {
                        const status = getStatusInfo(budget);
                        return (
                            <div key={budget.id} className="space-y-2">
                                <div className="flex items-center justify-between text-sm">
                                    <div className="flex items-center gap-2 min-w-0">
                                        {status.icon}
                                        <span className="font-medium truncate">
                                            {budget.name}
                                        </span>
                                    </div>
                                    <div className="flex items-center gap-2 shrink-0">
                                        <span className={cn(
                                            "px-2 py-0.5 rounded text-[11px] font-medium whitespace-nowrap",
                                            status.badge
                                        )}>
                                            {status.label}
                                        </span>
                                        <span className={cn("font-bold text-sm w-12 text-right tabular-nums", status.pct)}>
                                            {budget.utilization.toFixed(0)}%
                                        </span>
                                    </div>
                                </div>
                                <Progress
                                    value={Math.min(budget.utilization, 100)}
                                    indicatorClassName={status.bar}
                                    className="h-1.5"
                                />
                                <p className="text-[11px] text-muted-foreground">
                                    {formatCurrency(budget.spent)} spent
                                    {budget.isExceeded
                                        ? ` — over by ${formatCurrency(budget.overBy)}`
                                        : ` of ${formatCurrency(budget.amount)}`}
                                </p>
                            </div>
                        );
                    })}
                </div>
            </CardContent>
        </Card>
    );
}
